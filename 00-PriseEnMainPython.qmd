...
1. **N'installez par la dernière version de Python** : installez toujours une version ou deux qui précède la dernière version. Les versions trop récentes peuvent être instables. La version de python peut être spécifiée au moment de la création d'un environnement virtual (voir plus bas)
...
1. Option 1 : vous utilisez Anaconda ou Miniconda, dans ce cas la commande `conda` est utilisée pour créer un environnement test avec Python 3.9:
```
