...
for ax, K in zip(axs.flatten(), [1,3,7,15]):
    ...
    _ = disp.ax_.set_title(
        f"K={clf[-1].n_neighbors} erreur={(y_test != y_pred).sum()/X_test.shape[0]*100:3.1f}%"
    )
...
plt.show()
```
On peut observer que les frontières de décision sont sous forme d'ellipses.
...
fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(8, 6))
plt.imshow(y_classe, cmap=cmap_classes2)
ax.set_title("Carte d'occupation des sols la méthode Bayésienne naive")
plt.show()
```
### Analyse Discriminante Quadratique (ADQ)

L'analyse discriminante quadratique peut-être vue comme une généralisation de l'approche Bayésienne naive.
```{python}
...
for index, b in enumerate([0,1,3]):
  ...
  h.set_xlabel(noms_bandes[2])
  h.set_ylabel(noms_bandes[b])
```
