...
```{python}
%%capture
!pip install -qU matplotlib rioxarray
```
...
Le mélange de Gaussiennes est probablement la méthode statistique la plus simple. On suppose ici que les valeurs observées sont un mélange de distributions Gaussiennes.

La méthode d'estimation classique est la méthode (EM) `Expectation-Maximization` proposée par Dempster en 1977 [@Dempster-1977].
