...
L'égalisation d'histogramme consiste à modifier les valeurs des pixels d'une image cible de façon à ce que la distribution cumulée des valeurs (CDF) deviennent similaire à celle d'une image cible. La CDF (*Cumulative Distribution Function*) est simplement la somme cumulée des valeurs de l'histogramme:
...
On peut choisir $CDF_{cible}$ comme correspondant à une image où chaque valeur de pixel est équiprobable, ce qui veut dire $ hist_{cible}(j)=1/L$ avec $L$ est le nombre de valeurs possibles dans l'image (par exemple $L=256$).
$$
j = L \times CDF_{source}(i)
$$
On peut appliquer cette procédure sur l'image SAR de la façon suivante:
```{python}
