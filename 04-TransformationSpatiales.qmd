...
```
Au lieu d'appliquer un filtre moyen, le filtre de Lee applique une pondération:
```{python}
rolling_win = img_SAR.sel(band=2).rolling(x=5, y=5,  min_periods= 3, center= True).construct(x="x_win", y="y_win", keep_attrs= True)
filtre_moyen= rolling_win.mean(dim= ['x_win', 'y_win'], skipna= True)
...
axes[0].set_title("CV")
plt.subplot(1, 2, 2)
ponderation.plot.imshow( vmin=0, vmax=1) #cmap=plt.get_cmap('hot'),
...
xr.ufuncs.log10(filtered).plot.imshow() #cmap=plt.get_cmap('hot'),
axes[1].set_title("Pondération")
```
