...
from skimage import data, graph, segmentation, color
```
### Images utilisées
...
plt.show()
```
### Fusion des segments par graphe

Une segmentation peut produire beaucoup trop de segments. Un post-traitement possible est de fusionner les segments similaires. Un graphe d'adjacence de régions (voir @fig-rag) est formé à partir des segments connectés où chaque noeud représente un segment et un lien une proximité (@Jaworek-2018). À partir de ce graphe, on peut fusionner les noeuds similaires à partir de leur distance radiométrique.

![Graphe d'adjacence de régions (@Jaworek-2018)](images/Region-adjacency-graph.png "Graphe d'adjacence de régions"){#fig-rag}
